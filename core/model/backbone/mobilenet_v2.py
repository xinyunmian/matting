...
from core.backbone.basic import _ConvBNReLU, _DepthwiseConv, InvertedResidual
...
class MobileNet(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d):
        ...
        multiplier = 1.0  #cfg.MODEL.BACKBONE_SCALE
        conv_dw_setting = [
            [64, 1, 1],
            [128, 2, 2],
            [256, 2, 2],
...
class MobileNetV2(nn.Module):
    def __init__(self, num_classes=1000, norm_layer=nn.BatchNorm2d):
        ...
        output_stride = 32  #cfg.MODEL.OUTPUT_STRIDE
        self.multiplier = 1.0  #cfg.MODEL.BACKBONE_SCALE
        if output_stride == 32:
            dilations = [1, 1]
...
def mobilenetv1(norm_layer=nn.BatchNorm2d):
    return MobileNet(norm_layer=norm_layer)


def mobilenetv2(norm_layer=nn.BatchNorm2d):
